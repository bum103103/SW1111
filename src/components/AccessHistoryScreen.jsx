// src/components/AccessHistoryScreen.jsx
...
const AccessHistoryScreen = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const response = await fetch('/api/passwords/access-history', {
        headers: {
  ...
  const filteredHistory = history.filter(item => 
    ...
    item.user_nickname?.toLowerCase().includes(searchTerm.toLowerCase())
  );
            ...
            {filteredHistory.map((item) => (
              <div key={item.id} className="bg-white rounded-lg border p-4 space-y-2">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-sm text-blue-500">출입 성공</div>
                </div>
